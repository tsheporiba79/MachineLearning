...
def get_or_create_tokenizer(config, ds, lang):
...
def get_ds(config):
    ...
    for item in ds_raw:
        source_ids = tokenizer_source.encode(
            item["translation"][config["lang_source"]]
        ).ids
...
def train_model(config):
    ...
    if device == "cuda":
        ...
        print(
            "For Windows users with an NVidia GPU, you may find this video helpful:https://www.youtube.com/watch?v=r7Am-ZGMef8"
        )
        print(
            "To install the pre-release versions of PyTorch, torchvision, torchaudio, and torchtext on a Mac machine, you can use the following command: pip3 install --pre torch torchvision torchaudio torchtext --index-url https://download.pytorch.org/whl/nightly/cpu"
        )
